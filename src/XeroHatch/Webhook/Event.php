...
class Event
{
     ...
     *
     * @param \XeroHatch\Webhook $webhook
     * @param array $event event details
     *
     * @throws \XeroHatch\Application\Exception if the provided payload is malformed
     */
    public function __construct($webhook, $event)
    {
        ...
        foreach ($fields as $required) {
            if (! isset($event[$required])) {
                throw new \XeroHatch\Application\Exception("The event payload was malformed; missing required field {$required}");
            }
    ...
    public function getEventClass()
    {
        if ($this->getEventCategory() === 'INVOICE') {
            return \XeroHatch\Models\Accounting\Invoice::class;
        }
        if ($this->getEventCategory() === 'CONTACT') {
            return \XeroHatch\Models\Accounting\Contact::class;
        }
     ...
     *
     * @param \XeroHatch\Application $application an optional application instance to use to retrieve the remote resource.
     *                                          Useful if you have separate instances with different oauth tokens based on the tenant
     *
     * @return array|\XeroHatch\Remote\Model If the event category is known, returns the model, otherwise, returns the resource as an array
     */
